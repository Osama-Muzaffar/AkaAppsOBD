...
import com.akapps.obd2carscannerapp.Activity.MyMainActivity;
...
public abstract class CommService
{
	...
	synchronized void setState(STATE state)
	{
		...
		mHandler.obtainMessage(MyMainActivity.Constants.MESSAGE_STATE_CHANGE, state).sendToTarget();
	}
	...
	void connectionFailed()
	{
		...
		Message msg = mHandler.obtainMessage(MyMainActivity.Constants.MESSAGE_TOAST);
		Bundle bundle = new Bundle();
		bundle.putString(MyMainActivity.Constants.TOAST, mContext.getString(R.string.unabletoconnect));
		msg.setData(bundle);
	...
	void connectionLost()
	{
		...
		Message msg = mHandler.obtainMessage(MyMainActivity.Constants.MESSAGE_TOAST);
		Bundle bundle = new Bundle();
		bundle.putString(MyMainActivity.Constants.TOAST, mContext.getString(R.string.connectionlost));
		msg.setData(bundle);
		mHandler.sendMessage(msg);
	}
	 ...
	 */
	void connectionEstablished(String deviceName,String address)
	{
		// Send the name of the connectionEstablished device back to the UI Activity
		Message msg = mHandler.obtainMessage(MyMainActivity.Constants.MESSAGE_DEVICE_NAME);
		Bundle bundle = new Bundle();
		bundle.putString(MyMainActivity.Constants.DEVICE_NAME, deviceName);
		bundle.putString(MyMainActivity.Constants.DEVICE_ADDRESS, address);
		msg.setData(bundle);
		mHandler.sendMessage(msg);
