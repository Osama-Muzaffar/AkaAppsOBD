...
import com.akapps.obd2carscannerapp.Activity.MyRealTimeActivity;
import com.akapps.obd2carscannerapp.Models.BluetoothManager;
...
public class ObdGatewayService1 extends AbstractGatewayService {

    private static final String TAG = ObdGatewayService1.class.getName();
    ...
    public void startService() throws IOException {
        ...
        SharedPreferences prefs = getSharedPreferences("blue", MODE_PRIVATE);*/

        String restoredText = PairedDeviceSharedPreference.getInstance(getApplicationContext()).getBtDeviceAddress();
        Log.d(TAG, "startService: restoredText "+restoredText);
        String remoteDevice = null;

        if (restoredText != null) {
            remoteDevice = restoredText;//"No name defined" is the default value.
    ...
    private void startObdConnection() throws IOException {
        ...
        queueJob(new ObdCommandJob(new SelectProtocolCommand(ObdProtocols.valueOf(Constants.PROTOCOL))));
     ...
     * a job to the queue while setting its ID to the
    ...
    protected void executeQueue() throws InterruptedException {
        ...
        while (!Thread.currentThread().isInterrupted()) {
            ...
            if (job != null) {
                ...
                ((MyRealTimeActivity) ctx).runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ((MyRealTimeActivity) ctx).stateUpdate(job2);
                    }
                });
            }
        }
