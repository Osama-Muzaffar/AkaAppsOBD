...
public class AppPurchase {
    ...
    private ArrayList<QueryProductDetailsParams.Product> subscriptionList; // Subscription products
    private ArrayList<QueryProductDetailsParams.Product> inappList; // In-app products
    private List<PurchaseItem> purchaseList; // List of purchase items
    ...
    private boolean isProductDetailsRequired; // Product details queried flag
    private int purchaseType; // Type of purchase (in-app or subscription)
    ...
    private boolean isPurchased = false; // Purchase state flag
    private String purchaeProductId = ""; // Purchased product ID
    public List<PurchaseResult> purchaseResultList = new ArrayList<>(); // List of purchase results
    ...
    BillingClientStateListener billingClientStateListener = new BillingClientStateListener() {
        ...
        public void onBillingSetupFinished(@NonNull BillingResult billingResult) {
            ...
            if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                ...
                if (!AppPurchase.this.inappList.isEmpty()) {
                    AppPurchase.this.billingClient.queryProductDetailsAsync(
                            QueryProductDetailsParams.newBuilder().setProductList(AppPurchase.this.inappList).build(),
                            new ProductDetailsResponseListener() {
                                public void onProductDetailsResponse(BillingResult billingResult2, List<ProductDetails> productDetailsList) {
                                    if (productDetailsList != null) {
                                        ...
                                        AppPurchase.this.isProductDetailsRequired = true;
                ...
                if (!AppPurchase.this.subscriptionList.isEmpty()) {
                    QueryProductDetailsParams build = QueryProductDetailsParams.newBuilder().setProductList(AppPurchase.this.subscriptionList).build();
                    Iterator<QueryProductDetailsParams.Product> it = AppPurchase.this.subscriptionList.iterator();
                    while (it.hasNext()) {
    ...
    public void initBilling(Application application, List<String> listINAPId, List<String> listSubsId) {
        ...
        this.subscriptionList = a(listSubsId, "subs");
        this.inappList = a(listINAPId, "inapp");
    ...
    public void verifyPurchased(boolean isCallback) {
        Log.d(Tag, "isPurchased : " + this.subscriptionList.size());
        this.w = false;
        if (this.inappList != null) {
            this.billingClient.queryPurchasesAsync(
                    ...
                    (billingResult, list) -> {
                        ...
                        if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                            for (Purchase purchase : list) {
                                for (QueryProductDetailsParams.Product product : this.inappList) {
                    ...
                    (billingResult2, list2) -> {
                        ...
                        if (billingResult2.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                            for (Purchase purchase : list2) {
                                for (QueryProductDetailsParams.Product product : this.subscriptionList) {
                                    if (purchase.getProducts().contains(product.zza())) {
                                        a(
                                                new PurchaseResult(
                                                        purchase.getOrderId(),
    ...
    public String subscribe(Activity activity, String subsId) {
        if (this.productDetailsList1 == null) {
            ...
            String trialId = null;
            for (PurchaseItem item : this.purchaseList) {
    ...
    private void c(List<PurchaseItem> purchaseItems) {
        ...
        Log.d(Tag, "syncPurchaseItemsToListProduct: listINAPId " + this.inappList.size());
        this.subscriptionList = arrayList2;
        Log.d(Tag, "syncPurchaseItemsToListProduct: listSubscriptionId " + this.subscriptionList.size());
    }
    ...
    public String purchase(Activity activity, String productId) {
        ...
        this.u = productId;
        this.purchaseType = TYPE_IAP.PURCHASE;
        switch (this.billingClient.launchBillingFlow(
                activity,
                BillingFlowParams.newBuilder()
    ...
    public void initBilling(Application application, List<PurchaseItem> purchaseItemList) {
        ...
        this.purchaseList = purchaseItemList;
        c(purchaseItemList);
        BillingClient build = BillingClient.newBuilder(application)
                .setListener(this.purchasesUpdatedListener)
                .enablePendingPurchases()
    ...
    public void a(Purchase purchase) {
        double priceWithoutCurrency = getPriceWithoutCurrency(this.u, this.purchaseType);
        if (this.purchaseListener != null) {
            this.isPurchased = true;
